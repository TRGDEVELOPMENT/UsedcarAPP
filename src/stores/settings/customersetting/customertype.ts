...
export const customertypeStore = defineStore("customertype", () => {
  ...
  const getCustomerTypeList = async (
    ...
    await ApiService.get("/setting/customertype/list", { params: params })
      .then((data) => {
        res = data.data[0];
      })
      .catch(() => {
  ...
  const updateCustomerTypeById = async (
    ...
    await ApiService.update("/setting/customertype", "update", {
      data: [params],
    })
      .then(() => {
  ...
  const deletePrefixSettingById = async (
    ...
    ApiService.setHeader();
    const res: Resp = { message: "", status: false };
    await ApiService.update("/setting/customertype", "delete", {
      data: [params],
    })
      .then(() => {
  ...
  const insertCustomertypeById = async (
    ...
    ApiService.setHeader();
    const res: Resp = { message: "", status: false };
    await ApiService.post("/setting/customertype/insert", { data: [params] })
      .then(() => {
