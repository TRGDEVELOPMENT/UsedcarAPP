<template>
  ...
  <table class="table table-hover table-bordered border">
        ...
        <tbody v-if="listData['data']==undefined || listData['data'].length==0">
          <tr class="fs-7 text-gray-800"><td colspan="6" class="text-center">ไม่พบข้อมูล</td></tr>
        </tbody>
        <tfoot>
            ...
            <td colspan="5" class="text-end">
              <vue-awesome-paginate v-if="totalpage!=0" :total-items="totalpage"
              :items-per-page="10" :max-pages-shown="10"
              :show-jump-buttons="true" :on-click="onPageClick"
              v-model="currentpage"
              />
            </td>
          </tfoot>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="staticBackdropLabel">{{ titlemodel }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body" >
        <div class="mb-3">
          <label class="form-label">คำนำหน้าชื่อ</label>
          <input type="text" class="form-control" id="prefixinputbox" placeholder="นาย/ นาง/ นางสาว" v-model="selectdata.text">
        </div>
      </div>
      <div class="modal-footer">
        <button id="backbutton" type="button" class="btn btn-secondary" data-bs-dismiss="modal">กลับ</button>
        <button type="button" class="btn btn-primary" data-bs-dismiss="modal" @click="onSave">บันทึก</button>
      </div>
    </div>
  </div>
</div>
...
export default defineComponent({
  ...
  setup() {   
    ...
    const initial = async () =>{
      listData.value = await prefixstore.getPrefixSetting(key.value,1);
    ...
    const onSearch = async () => {
      ...
      if(keySearch!=''){
        const keyword = key.value.trim();
        listData.value = await prefixstore.getPrefixSetting(keyword,currentpage.value);
        totaldata.value = listData.value.totaldata;
        totalpage.value = totaldata.value;
      } else {
        initial();
